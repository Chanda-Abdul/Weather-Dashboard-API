    ...
    .then(data => {
      ...
      const saveCityWeather = () => {
        ...
        localStorage.setItem(
          "cityWeatherList",
          JSON.stringify(cityWeatherList)
        );
      ...
      currentFiveDayForecast.innerHTML = `${currentForecast
        .map(forecast => {
...
const displayCurrentWeatherData = () => {
  ...
  currentFiveDayForecast.innerHTML = cityWeatherList[0].forecast
    .map(day => {
      return `<div class="card five-day-forecast col-2">
            <h5 class="card-title">${day.dt_txt.slice(0, 10).split("-")[1]}/${
        day.dt_txt.slice(0, 10).split("-")[2]
      }/${day.dt_txt.slice(0, 10).split("-")[0]}</h5>
